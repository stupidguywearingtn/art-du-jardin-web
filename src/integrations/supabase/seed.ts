...
export async function ensureProjectTypesSeeded(): Promise<boolean> {
  ...
  if ((count ?? 0) === 0) {
    await supabase.from("project_types").insert([
      ...
      { slug: "parking", label: "Parking pro", description: "Voirie poids lourds et grand espace.", price_from: 55, price_unit: "€/m²", show_price: false, display_order: 3, active: true },
